...
static inline iopde_t iopde_new_invalid(void)
{
   ...
   pde = iopde_iopde_pt_new(0, 0, 0, 0);
...
static inline iopte_t iopte_arm_new(bool_t read, bool_t write, word_t paddr)
{
   ...
   int write_bit = (write ? 1 : 0);
   int read_bit = (read ? 1 : 0);
...
deleteIOPageTable(cap_t io_pt_cap)
{
    ...
    if (cap_io_page_table_cap_get_capIOPTIsMapped(io_pt_cap)) {
        ...
        if (isIOPDEValid(lu_ret.iopdSlot) &&
                ...
                iopde_iopde_pt_ptr_get_address(lu_ret.iopdSlot)
                != (pptr_to_paddr((void *)cap_io_page_table_cap_get_capIOPTBasePtr(io_pt_cap)))) {
            return;
        }
